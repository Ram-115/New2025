      ...
      <div className='flexRow fullH' style={{ padding: '30px', borderRadius: '10px' }}>
        ...
        <div className='flexCol jCntr aCntr halfWidth' style={{ border: '1px solid rgb(214, 216, 219)', borderRadius: '0px 10px 10px 0px' }}>
          ...
          <form onSubmit={handleSubmit} style={{ width: '60%' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}> <TextField label="Username" name="username" value={username} onChange={(e) => setUsername(e.target.value)} type="text" />
              <TextField label="Email" name="email" value={email} onChange={(e) => setEmail(e.target.value)} type="email"/>
              <TextField label="Password" name="password" value={password} onChange={(e) => setPassword(e.target.value)} type={showPassword ? "text" : "password"} InputProps={{
                endAdornment: (
              ...
              <TextField label="Confirm Password" name="confirmPassword" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} type={showPassword ? "text" : "password"} InputProps={{
                endAdornment: (
                  <InputAdornment position="end">
                    <IconButton>
                      {showPassword ? <Visibility onClick={() => setShowPassword(!showPassword)} /> : <VisibilityOff onClick={() => setShowPassword(!showPassword)} />}
          ...
          <span style={{ marginTop: '20px' }}>Already have an account? <Link style={{ color: '#0C5A96', textDecoration: 'none' }} to="/">Login</Link></span>
        </div>
      </div>
